...
class SolicitudCanceladaConsumerTest {
    ...
    @Test
    void shouldMapEventAndCallUseCaseWhenSolicitudCanceladaArrives() throws Exception {
        SolicitudCanceladaEvent evento = new SolicitudCanceladaEvent(
        ...
        assertEquals("solicitud.cancelada", notificacion.getEventoOrigen());
        assertEquals(EstadoNotificacion.PENDIENTE, notificacion.getEstado());
        assertEquals("Tu solicitud de vacaciones fue cancelada", notificacion.getAsunto());
        assertEquals(
                "Hola Ana Perez, tu solicitud de vacaciones del 2026-09-10 al 2026-09-15 fue cancelada.",
